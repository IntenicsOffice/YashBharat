...
const Login = ({navigation}) => {
  ...
  // const getCred = async () => {
  //   const temp1 = await getObjectData('user_credential_body');
  //   if (temp1 != null) {
  ...
  //   }
  // };
  const getTokenSet = async (mobile) => {
    const token = await messaging().getToken();
    const previousToken = await AsyncStorage.getItem('devicetoken');
    await AsyncStorage.setItem('devicetoken', token);
    console.log("token",token)
    console.log("previous token ",previousToken)
    if (token !== previousToken) {
     const data = {mobile: mobile, device_token: token , previousToken};
        updateUserDeviceToken(data);
  ...
  React.useEffect(() => {
    (async () => {
      await getData('login_token_status').then(res => {
        ...
        if (res) {
          // console.log('inside of if login', res);
